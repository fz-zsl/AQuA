...
def get_dataloader(args, scanqa, all_scene_list, split, config):
    ...
    return dataset, dataloader, answer_cands
...
def calc_variance(AL_Phi, logits):
    with torch.no_grad():
        ...
        # eigv, _ = torch.linalg.eig(torch.cov(AL_Phi.T, aweights=probs))
        # res = torch.sum(torch.log(torch.clamp(torch.real(eigv), min=1e-9)))
        # print(res)
    return res
...
def eval_qa(args):
    ...
    # dataloader
    _, dataloader, answer_cands = get_dataloader(args, scanqa, scene_list, "train" if args.use_train else "val", DC)
    ...
    # evaluate
    print("evaluating...")
    OUTPUT_SPLIT = "train" if args.use_train else "val"
    score_path = os.path.join(CONF.PATH.OUTPUT, args.folder, f"score.{OUTPUT_SPLIT}.pkl")

    # AL modified: calculate \Phi
    if args.AL_mode in ['similarity', 'variance']:
        fn = f'AL_Phi_cache_{len(answer_cands)}.pt'
        if os.path.exists(fn):
            ...
            bert_model = BertModel.from_pretrained(AL_BERT_MODEL)
            ...
            for text in tokenized_texts:
                with torch.no_grad():
                    outputs = bert_model(text).last_hidden_state
    ...
    pred_path = os.path.join(CONF.PATH.OUTPUT, args.folder, f"pred.{OUTPUT_SPLIT}.pkl")
        ...
        for trial, seed in enumerate(seeds):
            ...
            for data in tqdm(dataloader):
                ...
                with torch.no_grad():
                    ...
                    # AL modified
                    if args.AL_mode == 'variance':
                        _var = calc_variance_2D(AL_Phi, data["answer_scores"])[0]
                ...
                for i in range(ids.shape[0]):
                    ...
                    # AL modified, only holds for bs = 1
                    if args.AL_mode == 'variance':
                        predictions[scene_id][question_id]["variance"] = _var.item()
                        predictions[scene_id][question_id]["loss"] = _loss.item()
...
def eval_det(args):
    ...
    # dataloader
    dataset, dataloader, _ = get_dataloader(args, scanqa, scene_list, "train" if args.use_train else "val", DC)
    scanqa = dataset.scanqa
...
if __name__ == "__main__":
    ...
    parser.add_argument("--batch_size", type=int, help="batch size", default=1)  # 16
    ...
    # AL mode
    parser.add_argument("--AL_mode", type=str, help="AL_mode", default="variance")
