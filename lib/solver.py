...
class Solver():
    ...
    # def _update_mean(self, tp, data_mat):
    #     # print('[AL] Update mean:')
    #     # print(self.al_data_dict[f'sum_{tp}_feat'], data_mat)
    #     if self.al_data_dict[f'sum_{tp}_feat'] is None:
    #         self.al_data_dict[f'sum_{tp}_feat'] = torch.sum(data_mat, dim=0)
    #         self.al_data_dict[f'cnt_{tp}_feat'] = data_mat.shape[0]
    #     else:
    #         self.al_data_dict[f'sum_{tp}_feat'] += torch.sum(data_mat, dim=0)
    #         self.al_data_dict[f'cnt_{tp}_feat'] += data_mat.shape[0]
    ...
    def _feed(self, dataloader, phase, epoch_id):
        ...
        # # AL modified
        # if 'sum_lang_feat' not in self.al_data_dict.keys():
        #     print('[AL] Initialize AL feature logs.')
        #     self.al_data_dict['cnt_lang_feat'] = 0
        #     self.al_data_dict['cnt_object_feat'] = 0
        #     self.al_data_dict['cnt_fuse_feat'] = 0
        #     self.al_data_dict['sum_lang_feat'] = None
        #     self.al_data_dict['sum_object_feat'] = None
        #     self.al_data_dict['sum_fuse_feat'] = None
        #     self.al_data_dict['mean_lang_feat'] = None
        #     self.al_data_dict['mean_object_feat'] = None
        #     self.al_data_dict['mean_fuse_feat'] = None
        ...
        for data_dict in dataloader:
            ...
            for key in data_dict:
                ...
                elif type(data_dict[key]) is not list:  # AL modified
            ...
            with torch.autograd.set_detect_anomaly(True):
                ...
                # AL modified
                # with torch.no_grad():
                #     self._update_mean('lang', torch.mean(data_dict['lang_feat'], dim=1).squeeze(dim=1))
                #     self._update_mean('object', torch.mean(data_dict['object_feat'], dim=1).squeeze(dim=1))
                #     self._update_mean('fuse', data_dict['fuse_feat'])

                # # if self.al_data_dict['all_lang_feat'] is None:
                # #     self.al_data_dict['all_lang_feat'] = \
                # #         torch.mean(data_dict['lang_feat'], dim=1).squeeze(dim=1)
                # # else:
                # #     self.al_data_dict['all_lang_feat'] = torch.cat((
                # #         self.al_data_dict['all_lang_feat'],
                # #         torch.mean(data_dict['lang_feat'], dim=1).squeeze(dim=1)
                # #     ), dim=0)
                
                # # if self.al_data_dict['all_object_feat'] is None:
                # #     self.al_data_dict['all_object_feat'] = \
                # #         torch.mean(data_dict['object_feat'], dim=1).squeeze(dim=1)
                # # else:
                # #     self.al_data_dict['all_object_feat'] = torch.cat((
                # #         self.al_data_dict['all_object_feat'],
                # #         torch.mean(data_dict['object_feat'], dim=1).squeeze(dim=1)
                # #     ), dim=0)
                
                # # if self.al_data_dict['all_fuse_feat'] is None:
                # #     self.al_data_dict['all_fuse_feat'] = data_dict['fuse_feat']
                # # else:
                # #     self.al_data_dict['all_fuse_feat'] = torch.cat((
                # #         self.al_data_dict['all_fuse_feat'],
                # #         data_dict['fuse_feat']
                # #     ), dim=0)
            ...
            if phase == "train":
                ...
                if (self._global_iter_id + 1) % self.verbose == 0:
                    ...
                    # print(self.al_data_dict['sum_lang_feat'])
                ...
                if self._global_iter_id % self.val_step == 0:
                    ...
                    # # AL modified, save data and refresh mean after each evaluation
                    # # AL_LOG_DIR = os.path.join(".", 'outputs', 'AL_FEAT_LOGS')
                    # # torch.save(self.al_data_dict['all_lang_feat'], os.path.join(AL_LOG_DIR, 'all_lang_feat.pt'))
                    # # torch.save(self.al_data_dict['all_object_feat'], os.path.join(AL_LOG_DIR, 'all_object_feat.pt'))
                    # # torch.save(self.al_data_dict['all_fuse_feat'], os.path.join(AL_LOG_DIR, 'all_fuse_feat.pt'))
                    # with torch.no_grad():
                    #     if self._global_iter_id > 0:
                    #         self.al_data_dict['mean_lang_feat'] = self.al_data_dict['sum_lang_feat'] / self.al_data_dict['cnt_lang_feat']
                    #         self.al_data_dict['mean_object_feat'] = self.al_data_dict['sum_object_feat'] / self.al_data_dict['cnt_object_feat']
                    #         self.al_data_dict['mean_fuse_feat'] = self.al_data_dict['sum_fuse_feat'] / self.al_data_dict['cnt_fuse_feat']
                    #     self.al_data_dict['sum_lang_feat'] = None
                    #     self.al_data_dict['sum_object_feat'] = None
                    #     self.al_data_dict['sum_fuse_feat'] = None
                    #     self.al_data_dict['cnt_lang_feat'] = 0
